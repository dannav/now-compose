...
describe('toDockerCompose', () => {
  it('excludeKeys should exclude defined keys from objects', () => {
    const o = excludeKeys({ hello: 'world', excludeMe: '' }, ['excludeMe'])
    expect(o.excludeMe).not.toBeDefined()
  })

  it('linkServices should add linked services NOW_HOST_* and NOW_PORT_* to env vars', () => {
    const links = { api: ['db'], db: [] }
    const ports = { api: '8081:8080', db: '3306:3306' }
    const service = { name: 'api', config: { environment: {} } }

    const linked = linkServices(service, links, ports)

    expect(linked).toEqual({
      environment: { NOW_HOST_DB: 'host.docker.internal', NOW_PORT_DB: '3306' }
    })
  })
  ...
  it('should require at least docker-compose config version 3', () => {
    ...
    expect(() => {
      toDockerCompose(oldVersionConfig)
    }).toThrow()
  })
})
