...
const toDockerCompose = config => {
  ...
  if (configVersion < 3) {
    errors.exit(errors.general('now-compose.yml must be at least version 3'))
  }
  ...
  for (const serviceName in config.services) {
    ...
    // validate service names (since we use them for linking)
    const isValidName = RegExp(/^[\w\d_]+$/, 'g')
    if (!isValidName.test(serviceName)) {
      errors.exit(
        errors.general(
          `Invalid configuration. Service name '${serviceName}' contains invalid characters. Only letters, digits and '_' are allowed`
        )
      )
    }
    ...
    if (Object.keys(definition).indexOf('build') === -1) {
      errors.warn(errors.composeNoBuildTag(serviceName))
    }
        ...
        */
        ...
        if (Array.isArray(serviceConfig.ports)) {
          if (serviceConfig.ports.length > 1) {
            errors.warn(errors.composeSupportOnePort(serviceName))
          }

          if (!serviceConfig.ports.length) {
            errors.exit(errors.composeNoPortExposed(serviceName))
          } else {
            const ports = serviceConfig.ports.slice(0, 1)
            if (ports[0].indexOf(':') === -1) {
              errors.exit(errors.composeNoHostPort(serviceName))
            }
          ...
          if (serviceConfig.ports) {
            errors.exit(errors.composePortLongSyntaxNotSupported(serviceName))
          }
        }
    ...
    if (!foundContainerTag) {
      errors.warn(errors.composeDockerContainerNotDefined(serviceName))
    }
  }
...
module.exports = {
  toDockerCompose: toDockerCompose,
