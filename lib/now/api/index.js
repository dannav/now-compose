    ...
    try {
      ...
      stream.on('response', r => {
        ...
        if (r.statusCode === 403) {
          return rej(
            `
            Error: upload request returned 403 Forbidden.
      ...
      stream.on('uploadProgress', progress => {
        ...
        printUploadProgress(`Uploading ${relativeFilePath}`, progress.percent)
      })
...
function DeploymentError(name, body, statusCode) {
  this.message = `An error occured deploying service ${name}`
  this.body = body
