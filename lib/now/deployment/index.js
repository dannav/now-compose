    ...
    if (service['depends_on']) {
      if (Array.isArray(service['depends_on'])) {
        ...
        if (typeof service['depends_on'] === 'string') {
          ...
          console.log(
            `
            Error: depends_on property for service ${s} should be an Array or String.
            `
          )
...
const warnOnNonExistingLinks = (services, deploying) => {
  Object.keys(services).forEach(s => {
    ...
    if (!service.build) {
      console.log(
        `
        Warning: Service ${s} will not be deployed because it contains no build
    ...
    if (service['links'] && Array.isArray(service['links'])) {
      service['links'].forEach(l => {
        if (deploying.indexOf(l) == -1) {
          console.log(
            `
            Warning: Found link "${l}" for service "${s}". "${l}" is not a service
...
const deploy = async config => {
  ...
  Object.keys(deployed).forEach(sName => {
    ...
    console.log(`[${sName}] Available at url:  ${publicURL}`)
  })
